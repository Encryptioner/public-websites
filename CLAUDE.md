...
- **`raw-sources.js`** — the collapsed "Where do I get a raw file link?" list of paste services (Pastebin, Gist, Rentry, dpaste, 0x0.st) mounted inside the Any Page panel on both pages via `mountRawSources(el, context)`. Styles live in `panel.css` (`.raw-sources`). PrivateBin is deliberately absent — client-side-encrypted pastes can't serve plaintext at a URL.
...
- Events: home — `sites_loaded`, `sites_load_failed`, `site_searched`, `site_card_clicked`, `source_link_clicked`, `outbound_link_clicked`. Showcase & Blog — `showcase_loaded`, `showcase_filtered`, `showcase_card_clicked`, `blog_loaded`, `blog_filtered`, `blog_card_clicked`, `section_toggled` (`section: "showcase" | "blog" | "raw_sources"`, `expanded`). Shared raw-sources list (`raw-sources.js`, home bonus panel + any-page landing) — `raw_source_clicked` (`context`, `service`, `host`). Showcase + Blog share one chip-filter controller (`grid-filter.js` → `wireFilterableGrid()`); each section is just a config object in `showcase.js`/`blog.js`. Shared URL field — `url_field_engaged`, `url_field_cleared`, `url_field_copied`, `viewer_link_copied`, `viewer_opened`, `viewer_open_rejected` (all carry `context: "home" | "any_page"`). Any Page — `any_page_entered` (`mode`, `entry_format`), `page_rendered` (`via: direct|proxy`, `load_ms`, `redirected`), `page_render_failed`, `corner_toggled`, `corner_action`, `back_to_hub_clicked`. 404 — `not_found`, `not_found_recovered`.
